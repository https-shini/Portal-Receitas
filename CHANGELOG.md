...
- **Etapas seguintes do roadmap (§6.21) implementadas:** avaliação por estrelas **interativa** (upsert, um voto por usuário); **seguidores** (follow/unfollow + contador); **notificações** com sino ao vivo (Realtime) e página, geradas por **triggers no banco** (novo seguidor, comentário, avaliação); **denúncias** (botão) e **página de moderação** (RLS por papel `moderator`); Edge Functions `moderate-comment` e `recipe-stats`; **3 workflows n8n** exportados (`n8n/`); **Sentry** opcional (§6.17); **testes** (Vitest) e **CI** (`.github/workflows/web-ci.yml`).
- **Validado:** `typecheck` limpo, **Vitest** verde, `build` (Vite) com code-splitting (~138 kB gzip); as **3 migrations + seed** aplicam sem erro em **PostgreSQL 16 real** (11 tabelas, 25 policies RLS, 6 triggers, 8 funções). O app PHP e os 58 testes PHPUnit permanecem intactos.
- **Falta apenas o provisionamento externo** (criar o projeto Supabase, aplicar as migrations, configurar OAuth/buckets e hospedar o n8n) — nenhum código adicional é necessário para a fundação rodar.
...
- **Robustez + diagnóstico:** o entrypoint garante `/run/mysqld` (755, `mysql:mysql`) antes de subir o MariaDB — no plano free o `/run` é tmpfs vazio a cada boot — para o `www-data` alcançar a socket; e faz, no boot, uma checagem do caminho real da aplicação (`www-data` → socket → `tcc_receitas`) cujo resultado vai para o log. O `PdoConnectionFactory` passou a registrar no log do servidor (stderr → logs da Render) o motivo real de uma falha de conexão (sem expor senha), já que os entrypoints só mostram um 503 genérico.
...
- **Logout do menu consertado (H4):** o entrypoint de logout foi movido do caminho legado `assets/php/logout.php` para `public/logout.php` (docroot), e o link do menu do usuário passou a apontar para `logout.php`. O `AuthController::logout()` agora redireciona para `index.php` (o catálogo é público). Removido o arquivo legado órfão. Validado E2E: `logout.php` encerra a sessão (302 → `/index.php`; `me` volta 401).
